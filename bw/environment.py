    ...
    def decorator(func: Callable[[str], str]) -> Callable:
        ...
        def wrapper(*args, **kwargs) -> Any:
            try:
                if require:
                    GC.require(entry)
                return type_converter(cast(Any, func)(*args, **kwargs, key=entry))
...
class Environment:
    ...
    def orientor_role(self, key: str) -> int:
        return GC[key]

    @config_fetch('awaiting_orientation_role_id', int)
    def awaiting_orientation_role(self, key: str) -> int:
        return GC[key]

    @config_fetch('recruit_role_id', int)
    def recruit_role(self, key: str) -> int:
        return GC[key]

    @config_fetch('member_role_id', int)
    def member_role(self, key: str) -> int:
        return GC[key]
    ...
    @config_fetch('cron_channel_id', int)
    def cron_channel_id(self, key: str) -> int:
        return GC[key]
    ...
    def backend_port(self, key: str) -> int:
        return GC[key]
    ...
    def local_session_time(self) -> datetime.timedelta:
        return datetime.timedelta(hours=int(GC.get('local_session_time', 12 + 7)))
...
class Test(Environment):
    ...
    def discord_oauth_redirect_uri(self):
        return 'https://staging.bourbonwarfare.com/auth/login/discord/bot'
...
class Production(Environment):
    ...
    def discord_oauth_redirect_uri(self):
        return 'https://hub.bourbonwarfare.com/auth/login/discord/bot'
...
if GC.get('environment', 'local') == 'prod':
    ...
    logger.info('Using staging environment')
    ENVIRONMENT = Test()
else:
