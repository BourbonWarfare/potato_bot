...
class ArmaApi:
    def create_session_message(self, state: State, session_id: UUID, message_id: DiscordSnowflake):
        with state.Session.begin() as session:
            new_session = ArmaSessionMessage(session_id=session_id, message_id=int(message_id), has_coop_ended=False)
    ...
    def session_notification_message(self, state: State, session_id: UUID) -> DiscordSnowflake:
        with state.Session.begin() as session:
            query = select(ArmaSessionMessage).where(ArmaSessionMessage.session_id == session_id)
            arma_session = session.execute(query).scalar()
            if not arma_session:
            ...
            return DiscordSnowflake(arma_session.message_id)
